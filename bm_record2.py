...
def receive_new_frame(data_dict):
    ...
    if recording and data_dict["frame_number"] % 8 == 4:
        ...
        if success:
            cv2.imwrite("frames/frame_%d.jpeg" % data_dict["frame_number"], image)
        else:
...
def receive_rigid_body_frame( new_id, position, rotation ):
    if recording:
        frame_num += 1
        out = [frame_num] + position + rotation
        with open('optitrack_data/optitrack_data_untimed.csv', 'a', newline='') as f:
...
def file_setup():
    with open('optitrack_data/optitrack_data_untimed.csv', 'w', newline='') as f:
        f_writer = csv.writer(f)
        f_writer.writerow(["Pos X", "Pos Y", "Pos Z", "Rot X", "Rot Y", "Rot Z", "Rot W"])
        f.close()

# THIS ASSUMES 240HZ FRAMERATE
def file_add_timestamp():
    framerate = 240.0
    with open('optitrack_data/optitrack_data_untimed.csv') as old, open('optitrack_data/optitrack_data_timed.csv', 'w', newline='') as new:
        reader = csv.reader(old)
        rows = list(reader)
        writer = csv.writer(new)
        for row in rows[1:]:
            rownum = rows.index(row) - 1
            time = ((8 * rownum) + 4) / 240.0
            newrow = [time, row[1], row[2], row[3], row[4], row[5], row[6], row[7]]
...
if __name__ == "__main__":
    ...
    optionsDict["serverAddress"] = "192.168.2.109"
